...
public  extension UIImageView {
    ...
    func savedUIImageForUrl (_ url: String, block: @escaping (UIImage?, Bool) -> Swift.Void) {
        let fileManager = FileManager.default
        let imagePAth = (self.getDirectoryPath() as NSString).appendingPathComponent(url.imageName())
        if fileManager.fileExists(atPath: imagePAth) {
    ...
    func savedUIImage (_ name: String?,
    ...
    func saveUIImage (_ name: String?,
                             _ img: UIImage?) {
        if name != nil && img != nil {
    ...
    func saveUIImageForUrlSVG (_ url: String?, _ data: Data?) {
        if url != nil {
            var filename = url?.imageName()
            filename = filename?.replacingOccurrences(of: ".jpg", with: ".svg")
            if data != nil {
    ...
    func downloadUIImage (_ url: String?,
    ...
    @objc func downloadUIImageThread (_ mdd: NSMutableDictionary) {
